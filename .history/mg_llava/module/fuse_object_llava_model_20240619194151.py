        ...
        **kwargs,
        ...
        if not fuse_model:
            ...
            self.projector = Resampler(
                grid_size=24,
                embed_dim=self.llm.config.hidden_size,                      # 保持与llm输入的embed_dim 一致
                num_heads=self.visual_encoder.config.hidden_size // 128,  # 保持与视觉模型输出的 num_heads 一致
                kv_dim=self.visual_encoder.config.hidden_size,               # 保持与视觉模型输出的 kv_dim 一致
            ).to(self.visual_encoder.dtype)
            self.fuse_module = DualPathFuseModule4(
                low_res_dim=self.visual_encoder.config.hidden_size,
                high_res_dim=1536,
            )
...
class DualPathFuseModule4(nn.Module):
    # concat
    def __init__(self, low_res_dim, high_res_dim, zero_init=True):
        super().__init__()
        self.slow_conv = nn.Conv2d(high_res_dim, high_res_dim, 1)
