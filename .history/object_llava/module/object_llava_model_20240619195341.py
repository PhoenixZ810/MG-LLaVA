...
class ObjectLLaVAModel(BoxLLaVAModel):
    def __init__(self,
                 *args,
                 pretrained_pth=None,
                 projector_depth=2,
            ...
            for i, (boxes, labels) in enumerate(zip(data['gt_boxes'], data['gt_labels'])):
                ...
                if is_video:
                    ...
                    for j, (boxes_frame, labels_frame) in enumerate(zip(boxes, labels)):
                        out_box_feat_frame = self.align_box(j, visual_outputs_aux[i], boxes_frame, pixel_values, multi_level)
    ...
    def align_box(self, i, visual_outputs_aux, boxes, pixel_values, multi_level=False):
        ...
        if multi_level:
            ...
            for level, channels_loc in enumerate(multi_level):
                level_feature = visual_outputs_aux[
                    i : i + 1, channels : channels + channels_loc, :, :
                ]
